...
def test_extract_simple():
    ...
    assert page is None, f"Expected None, got {page}"
...
def test_encode_simple():
    filename = "photo.jpg"
    result = encode_metadata_in_filename(filename, preferred_size=3.45)
...
def test_encode_safecontainer():
    ...
    result = encode_metadata_in_filename(filename, preferred_size=1.5)
...
def test_encode_long_filename():
    ...
    result = encode_metadata_in_filename(filename, preferred_size=2.35)
...
def test_roundtrip():
    ...
    size = 2.35
    encoded = encode_metadata_in_filename(original, preferred_size=size)
    decoded_name, decoded_size, decoded_page = extract_metadata_from_filename(encoded)
    ...
    assert decoded_page is None, f"Expected None page, got {decoded_page}"
...
if __name__ == '__main__':
    ...
    def test_none_filename(self):
        base, size, page = extract_metadata_from_filename(None)
        assert base is None
        assert size is None
        assert page is None
    
    def test_empty_filename(self):
        base, size, page = extract_metadata_from_filename("")
        assert base == ""
        assert size is None
        assert page is None
...
class TestEncodePreferredSize:
    ...
    def test_integer_size(self):
        filename = "photo.jpg"
        result = encode_metadata_in_filename(filename, preferred_size=6.0)
        assert result == "photo-sz6.jpg"
    
    def test_trailing_zeros_removed(self):
        filename = "photo.jpg"
        result = encode_metadata_in_filename(filename, preferred_size=1.00)
    ...
    def test_none_filename(self):
        result = encode_metadata_in_filename(None, preferred_size=3.45)
        assert result is None
    
    def test_empty_filename(self):
        result = encode_metadata_in_filename("", preferred_size=3.45)
        assert result == ""
...
class TestRoundTrip:
    ...
    def test_complex_filename_with_underscores_and_uuid(self):
        ...
        # Roundtrip
        re_encoded = encode_metadata_in_filename(decoded_name, preferred_size=decoded_size)
...
# Run standalone tests if executed directly (pytest class tests run via pytest)
